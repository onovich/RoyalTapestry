...
Current status: basic locked combo protection is implemented. Clicking a scored row, column, or diagonal locks/unlocks that combo, and the cards captured by the lock cannot be moved away or replaced by another card. Empty cells are not locked.

Future design for optional in-line reorder inside locked combo cards. Do not implement this reorder extension yet.
...
- Locking is based on the cards that formed a scoring combo at the moment the row, column, or diagonal score box was locked.

## Juicier Interaction Feedback

Initial pass implemented.

- Dragged cards show a bright target edge when hovering over a valid board or hand drop target.
- Cards get a short settle animation on successful placement or return.
- Locked score boxes pop when locking, and locked cards shake when the player tries to move them.
...
- For locked combos, add a fade-out when unlocking.
